...
export default function FairBounty() {
  ...
  useEffect(() => {
    DbAPI.getStats().then((stats) => { if (stats) setGlobalStats(stats); }).catch(() => {});
    DbAPI.getBounties().then((bounties) => { if (Array.isArray(bounties)) setLiveBounties(bounties); }).catch(() => {});
    DbAPI.getCompletedBounties().then((b) => { if (Array.isArray(b)) setCompletedBountiesList(b); }).catch(() => {});
    DbAPI.getPublicProfiles().then((p) => { if (Array.isArray(p)) setCommunityProfiles(p); }).catch(() => {});
    DbAPI.getLeaderboard().then((l) => { if (Array.isArray(l)) setLeaderboardData(l); }).catch(() => {});
  }, []);
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          ...
          <div style={{ maxWidth: "800px", margin: "0 auto", padding: "24px 16px", textAlign: "center" }}>
            ...
            <div className="stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "24px", marginTop: "60px", ...fadeIn, transitionDelay: "0.5s" }}>
              {[
                { value: String(globalStats?.connectedWallets || 0), label: t.connectedWallets, live: true },
                { value: String(liveBounties?.length || globalStats?.bountyApps || 0), label: t.liveBounties, live: true },
                { value: String(globalStats?.profiles || 0), label: t.profiles, live: true },
              ].map((stat) => (
          ...
          <div style={{ ...cardStyle, ...fadeIn }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: "12px", marginBottom: "24px" }}>
              {[
                ...
                { label: lang === "es" ? "Envíos" : "Submissions", value: selectedBountySubmissions.length, color: "#888" },
            ...
            {(b.submissionRequirements || b.submission_requirements || b.evaluationCriteria || b.evaluation_criteria) && (
            ...
            {/* Share Bounty */}
            <div style={{ ...cardStyle, padding: "16px 20px", marginBottom: "24px", background: `${theme.primary}05` }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: "900px", margin: "0 auto", padding: "20px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", flexWrap: "wrap", gap: "12px", ...fadeIn, transitionDelay: "0.2s" }}>
          <div>
            ...
            <p style={{ fontSize: "12px", color: "#888" }}>{filteredBounties.length} bounties</p>
          </div>
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {/* Filter by tier */}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px", ...fadeIn, transitionDelay: "0.3s" }}>
          {filteredBounties.map((b) => {
            ...
            const isMyBounty = fullAddress && b.poster === fullAddress;
            return (
              <div key={b.id} style={{ ...cardStyle, cursor: "pointer", opacity: eligible ? 1 : 0.55, position: "relative", overflow: "hidden", border: `1px solid ${theme.primary}30` }}
