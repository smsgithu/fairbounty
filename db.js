...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "check-beta-app") {
      const { wallet } = req.query;
      if (!wallet) return res.json({ exists: false });
      try {
        const rows = await sql`SELECT id FROM fb_bounty_apps WHERE wallet = ${wallet} AND form_data->>'type' = 'beta_request' LIMIT 1`;
    ...
    // ============================================================
    // PUBLIC DATA (community page)
    // ============================================================
    if (action === "get-public-profiles") {
      try {
        const profiles = await sql`SELECT wallet, profile, updated_at FROM fb_profiles ORDER BY updated_at DESC LIMIT 200`;
    ...
    if (action === "get-leaderboard") {
      try {
        const rows = await sql`
          SELECT p.wallet, p.profile,
            b.bxp,
            COALESCE((b.bxp->>'welcome')::int, 0) + COALESCE((b.bxp->>'referrals')::int, 0) + COALESCE((b.bxp->>'referred')::int, 0) + COALESCE((b.bxp->>'submissions')::int, 0) + COALESCE((b.bxp->>'wins')::int, 0) as total_bxp
          FROM fb_profiles p
          LEFT JOIN fb_bxp b ON b.wallet = p.wallet
          ORDER BY total_bxp DESC NULLS LAST
          LIMIT 10
        `;
    ...
    if (action === "get-completed-bounties") {
      try {
        const bounties = await sql`
          SELECT b.*, s.display_name as winner_name, s.wallet as winner_wallet, s.tier as winner_tier, s.content as winner_content, s.links as winner_links, s.score as winner_score
          FROM fb_bounties b
          LEFT JOIN fb_submissions s ON s.id = b.winner_submission_id
    ...
    if (action === "create-bounty") {
      ...
      await sql`
        CREATE TABLE IF NOT EXISTS fb_bounties (
          id SERIAL PRIMARY KEY, title TEXT NOT NULL, description TEXT, project_name TEXT,
          category TEXT, prize_type TEXT DEFAULT 'USDC', reward TEXT, currency TEXT DEFAULT 'USDC',
          meme_token TEXT, nft_mint TEXT, nft_name TEXT, nft_image_url TEXT,
          min_tier INTEGER DEFAULT 1, tags JSONB DEFAULT '[]', deadline TEXT,
          poster TEXT, poster_name TEXT, poster_tier INTEGER,
          status TEXT DEFAULT 'open', contact_method TEXT, contact_value TEXT,
          submission_requirements TEXT, evaluation_criteria TEXT,
          submission_count INTEGER DEFAULT 0, winner_submission_id INTEGER,
          is_beta BOOLEAN DEFAULT true, created_at TIMESTAMP DEFAULT NOW(), updated_at TIMESTAMP DEFAULT NOW()
        )
      `;
      ...
      try {
        const rows = await sql`
          SELECT *, project_name as "projectName", prize_type as "prizeType",
            ...
            min_tier as "minTier", submission_count as "submissionCount",
            ...
            nft_image_url as "nftImageUrl", nft_name as "nftName2",
            is_beta as "isBeta", created_at as "createdAt"
          FROM fb_bounties WHERE status = 'open' ORDER BY created_at DESC LIMIT 100
        `;
        return res.json(rows);
      } catch (e) { return res.json([]); }
    }
    ...
    if (action === "submit-work") {
      ...
      await sql`
        CREATE TABLE IF NOT EXISTS fb_submissions (
          id SERIAL PRIMARY KEY, bounty_id TEXT NOT NULL, wallet TEXT NOT NULL,
          display_name TEXT, tier INTEGER DEFAULT 1, content TEXT NOT NULL, links TEXT,
      ...
      `;
      const existing = await sql`SELECT id FROM fb_submissions WHERE bounty_id = ${String(bountyId)} AND wallet = ${wallet} LIMIT 1`;
    ...
    if (action === "get-submissions") {
      ...
      try {
        const rows = await sql`
          ...
          FROM fb_submissions WHERE bounty_id = ${String(bountyId)} AND status = 'active'
    ...
    if (action === "vote") {
      ...
      await sql`
        CREATE TABLE IF NOT EXISTS fb_votes (
          id SERIAL PRIMARY KEY, submission_id INTEGER NOT NULL, voter_wallet TEXT NOT NULL,
          vote_type TEXT NOT NULL, vote_weight INTEGER DEFAULT 1,
          created_at TIMESTAMP DEFAULT NOW(), updated_at TIMESTAMP DEFAULT NOW(),
          UNIQUE(submission_id, voter_wallet)
        )
      `;
      const existing = await sql`SELECT id, vote_type, vote_weight FROM fb_votes WHERE submission_id = ${submissionId} AND voter_wallet = ${voterWallet} LIMIT 1`;
      if (existing.length > 0) {
        ...
        if (voteType === "up") {
          await sql`UPDATE fb_submissions SET score = score + ${safeWeight}, upvotes = upvotes + 1 WHERE id = ${submissionId}`;
        } else {
          await sql`UPDATE fb_submissions SET score = score - ${safeWeight}, downvotes = downvotes + 1 WHERE id = ${submissionId}`;
        }
      ...
      await sql`INSERT INTO fb_votes (submission_id, voter_wallet, vote_type, vote_weight, created_at) VALUES (${submissionId}, ${voterWallet}, ${voteType}, ${safeWeight}, NOW())`;
      if (voteType === "up") {
        await sql`UPDATE fb_submissions SET score = score + ${safeWeight}, upvotes = upvotes + 1 WHERE id = ${submissionId}`;
      } else {
        await sql`UPDATE fb_submissions SET score = score - ${safeWeight}, downvotes = downvotes + 1 WHERE id = ${submissionId}`;
    ...
    if (action === "select-winner") {
      ...
      await sql`UPDATE fb_bounties SET winner_submission_id = ${submissionId}, status = 'completed', updated_at = NOW() WHERE id = ${parseInt(bountyId)}`;
      await sql`UPDATE fb_submissions SET status = 'winner' WHERE id = ${submissionId}`;
      ...
      if (winnerRows.length > 0) {
        ...
        const multipliers = { 1: 1.0, 2: 1.25, 3: 1.5, 4: 2.0, 5: 3.0 };
        const winBxp = Math.floor(100 * (multipliers[tier] || 1.0));
    ...
    if (action === "admin-get-all") {
      ...
      // Also fetch beta rows for admin profiles tab
      let betaRows = [];
      try { betaRows = await sql`SELECT * FROM fb_beta_access ORDER BY added_at DESC`; } catch(e) {}
      return res.json({ bounties, apps, profiles, bxpRows, submissions, betaRows });
    }
